...
class PMCDownloader:
    def __init__(self, download_dir="data/pdf", batch_size=20, max_concurrent=10):
    ...
    async def _download_pdf_file(self, session, pdf_url, pmid):
        """실제 PDF 파일 다운로드"""
        try:
            filename = f"{pmid}_pmc.pdf"
            filepath = self.download_dir / filename
    ...
    async def _update_database(self, pmid, pmcid, pdf_url):
        ...
        try:
            ...
            log_debug(f"쿼리: UPDATE papers SET pdf_path = {local_path}, pdf_source = 'PMC', updated_at = NOW() WHERE pmid = {pmid}")
            cur.execute("""
                    UPDATE papers 
