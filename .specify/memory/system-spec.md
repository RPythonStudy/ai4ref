# ai4ref L1 System Spec (프로젝트 최상위 명세)

> **대상**: AI agent·개발자 (사람 대상 소개·사용법은 루트 `README.md`).
> **계층**: 이 문서 = L1(프로젝트). 피처 = `specs/<NNN-feature>/`(L2), 컴포넌트 = 코드 옆
> `<모듈>_spec.md`(L3). 설계 원칙 = `constitution.md`(동일 폴더).
> **용도**: 컨텍스트 주입용 정본 — 기능 슬라이스는 `001`~`011` 앵커로 부분 로딩.

시스템 = 두 트랙. **감시(alert)** = 진료지침 PICO 기준 신규 문헌 매일 감시 → 랜드마크
선별 알림·보관(§001~008). **수집(collect)** = 자연어 주제 대량 검색·본문 확보 → Zotero·
2nd-brain 배포(§009~011). 둘 다 결정론 backbone + 최소 LLM, 추가비용 0.
...
원칙 전문은 `constitution.md`. 아래는 L1 레벨 운영 정책 요약.
...
### 4. config·데이터 거버넌스
운영 config = 감시(`key_questions.yml`) + 수집 주제(term-based) + `features.yml`(YAML).
`state/`(seen-set)·`.env`(시크릿)은 gitignore. 패키징 = `pyproject.toml`(uv·slim deps).
