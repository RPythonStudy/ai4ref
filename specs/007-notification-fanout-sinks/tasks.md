...
- [x] T001 [P] `tests/unit/test_notify.py` 단위테스트 파일 생성 — `urllib.request.urlopen`/
...
- [x] T002 [US1] `src/notify/registry.py`·`base.py` fan-out 경로 동작 보존 확인 — `fan_out(item,
  sinks)` 가 통로별 `{name: ref|None}` 반환, `Sink` 인터페이스(available·send) 일관(FR-001·004).
- [x] T003 [US1] `src/notify/stdout_sink.py` `render` 보존 확인 — 제목·메타(저널·연도)·한 줄 근거·
  PubMed 링크 포함, 내부 전용 `abstract` 미노출(FR-006). `LandmarkItem.url` 자동 파생(FR-011).
- [x] T004 [P] [US1] `tests/unit/test_notify.py` — render 출력에 title·journal·reason·url 포함·
...
- [x] T005 [US2] `src/notify/registry.py` `build_sinks` 보존 확인 — `enabled` 토글 필터 +
  `available()` prereq 점검 → 미충족 skip·`log.warning`(FR-002·003).
- [x] T006 [US2] `src/notify/telegram_sink.py`·`zotero_sink.py` `available()` 보존 확인 — 토큰/
  키 부재·`pyzotero` 미설치 시 `(False, 사유)` 반환(fail-soft, FR-003·010).
- [x] T007 [P] [US2] `tests/unit/test_notify.py` — 토큰 빈 telegram = skip·stdout 활성 단언 +
...
- [x] T008 [US3] `src/notify/registry.py` `fan_out` 전송 격리 보존 확인 — `try/except` 로 통로
  실패 시 `out[name]=None`·`log.error`, 다른 통로 계속(FR-005).
- [x] T009 [US3] `src/notify/zotero_sink.py` 보존 확인 — 지정 컬렉션(기본 ai4ref-landmarks) 생성/
  탐색·`reason`→Extra 메모 보강·`collector.zotero_add` 재사용(FR-008). 새 통로 = `SINK_CLASSES`
  한 줄 확장 지점 명시(FR-009).
- [x] T010 [P] [US3] `tests/unit/test_notify.py` — sink A 예외·sink B 성공 시 결과
  `{A:None, B:ref}`·B 호출됨 단언(전송 격리, FR-005).

## Phase 6: Polish & 등가 검증

- [x] T011 `src/notify/notify_spec.md` 작성 (L3 ≤200줄) — Sink 계약(available·send)·build_sinks·
  fan_out·fail-soft 2종·플러그 확장(SINK_CLASSES)·LandmarkItem·FR001~011 참조.
- [x] T012 `src/alert/run.py` 호출부 점검 — `build_sinks`/`fan_out` 사용·실수집+stub 백엔드
  외부통로 차단 안전장치 보존(§006/§007 경계). 동작 변화 없음 단언.
- [x] T013 `uv run python -m pytest -q` 전체 통과 확인(기존 44 + 신규 test_notify).
- [x] T014 [P] 헌법 XI 줄수 점검 — `notify/*.py` 각 ≤300줄, `notify_spec.md` ≤200줄.
